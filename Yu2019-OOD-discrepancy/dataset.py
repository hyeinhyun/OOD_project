...
class label_data(data.Dataset):
    def __init__(self,sup_path,transform=None,crop=False):
        #self.ood = OOD_path
        #self.id = ID_path
        ...
        try:
            for c in range(1,num_class+1):
                ...
                for name in (os.listdir(class_path)):
                    img=Image.open(class_path+name).covert(mode='RGB')
                    if crop:
                            img = transform(img)
                    np_img = np.array(img)
...
class unlabel_data(data.Dataset):
    def __init__(self,id_path,ood_path,transform=None,crop=False):
        #self.ood = OOD_path
        #self.id = ID_path
        self.transform = transform
        self.data = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sunp_path',type=str,default='./dataset/train/sup/',help='dataset path for sup experiment')
    ...
    if args.api:
        ...
        sup_data=label_data(sup_path,transform=None,args.crop=False)
        #sup_val=label_data('val',args.num_class,transform=None,args.crop=False)
        #sup_test=label_data('test',args.num_class,transform=None,args.crop=False)
        ...
        unsup_data=unlabel_data(args.id_path,args.ood_path,data_transform,args.crop) 
